...
vector<int> BestJobAssignment2(const vector<Resources>& jobs,
  ...
  // Contraintes de dependances locales. On les repète pour chaque machine.
  for (int m = 0; m < num_machines; ++m) {
    for (const pair<int, int>& dep : local_dep) {
      // "Si job A tourne, alors job B tourne" -> c'est la contrainte B >= A
      MPConstraint* ct = solver.MakeRowConstraint(0, solver.infinity(), "");
      ct->SetCoefficient(var[dep.first][m], -1.0);
      ct->SetCoefficient(var[dep.second][m], 1.0);
    }
  }
