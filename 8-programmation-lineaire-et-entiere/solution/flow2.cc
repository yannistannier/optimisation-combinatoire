...
vector<FlowOnArc> BestFlow2(int num_nodes, const vector<BackboneArc>& backbone,
  ...
  // On ajoute les variables et contraintes de sur-utilisation.
  // Elles remplacent les anciennes contraintes de capacité.
  // (sur-utilisation + max_free_utilization_ratio) * capacity >= sum(flow)
  for (int a = 0; a < num_arcs; ++a) {
    MPVariable* var = solver.MakeNumVar(0, 1.0 - max_free_utilization_ratio,
                                        "OverUtilization" + std::to_string(a));
    objective->SetCoefficient(var, -penalty_cost);
    MPConstraint* ct = solver.MakeRowConstraint(
        0, backbone[a].capacity * max_free_utilization_ratio,
        "Utilization" + std::to_string(a));
    ct->SetCoefficient(var, -backbone[a].capacity);
    for (int d = 0; d < num_demands; ++d) {
